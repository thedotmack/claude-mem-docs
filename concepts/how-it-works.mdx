...
<CardGrid cols={3}>
  ...
  <Card title="session-end" icon="stop">
    (Optional) Saves before `/clear` when enabled
  </Card>
...
The hook reads `~/.claude-mem/claude-mem-index.jsonl`, so you always see the most recent summaries even without running a Chroma query.

## Memory Storage

claude-mem writes each summary to a local JSONL index and asks Claude (via MCP) to persist the same document in Chroma:
...
<FileTree>
  <Folder name="~/.claude-mem" defaultOpen>
    <File name="claude-mem-index.jsonl" />
    <Folder name="chroma" defaultOpen>
      <Comment>Persistent data for the Chroma MCP server</Comment>
    </Folder>
    ...
    <Folder name="hooks">
      <Comment>Installed Claude Code hook scripts</Comment>
    </Folder>
    <Folder name="logs">
      <Comment>Compression and hook logs</Comment>
    </Folder>
  </Folder>
</FileTree>
