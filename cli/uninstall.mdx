...
Removes claude-mem hooks from Claude Code settings while preserving your memory data and compressed transcripts.
...
The `uninstall` command safely removes claude-mem integration:
...
<Note>
Uninstalling removes the integration but preserves all your compressed transcripts and memory data. You can reinstall at any time without losing data.
</Note>
...
**Sample Output:**
```
🔄 Uninstalling Claude Memory System hooks...
...
The Claude Memory System hooks have been removed from your settings.
...
The command removes all three hook types:
...
The command can target different settings files:
...
Before modifying any settings file, a timestamped backup is created:
...
The command intelligently detects claude-mem hooks by looking for:
...
This ensures only claude-mem hooks are removed, leaving other hooks intact.
...
The command performs thorough cleanup:
...
All your preserved data will be immediately available.
