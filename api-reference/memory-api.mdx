...
Claude-mem uses the Model Context Protocol (MCP) to provide standardized memory operations through ChromaDB. This API allows you to store, retrieve, and search through conversation memories programmatically.

## MCP Client Interface

### IMCPClient

The core interface for memory operations:
...
Memory documents follow a standardized format:
...
Search operations return structured results:
...
Claude-mem provides these MCP tools for memory operations:
...
When memories are created through compression, they follow this structure:
...
The compression process produces structured results:
...
Memory operations can throw specific errors:
...
The compression API automatically handles memory storage:
...
Store custom memories outside of transcript compression:
