...
Claude-mem provides a comprehensive set of APIs for integrating memory compression and transcript processing into your applications. This guide covers the main API surfaces and how to use them programmatically.

## Core API Components

### 1. Memory Storage & Retrieval APIs

Claude-mem uses the Model Context Protocol (MCP) for memory operations, providing a standardized interface for vector database operations through ChromaDB.
...
The compression system transforms Claude Code conversation transcripts into searchable memory documents.
...
Event-driven hooks that integrate with Claude Code's lifecycle events.
...
Centralized configuration management for paths, settings, and project detection.
...
Claude-mem follows a layered architecture:
...
Claude-mem stores configuration in `~/.claude-mem/settings.json`:
...
All APIs use typed error classes:
...
Claude-mem is written in TypeScript with full type definitions included:
