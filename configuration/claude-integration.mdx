...
Claude-mem provides seamless integration with Claude Code through an advanced hook system and MCP (Model Context Protocol) server. This integration enables automatic memory compression, context loading, and intelligent session management without disrupting your workflow.
...
### 1. Hook System
Claude-mem integrates through Claude Code's native hook system, providing:
...
<Tabs>
  ...
  <Tab title="Minimal Context">
    ```
    🧠 Loaded 3 memories from previous sessions for new-project

    💡 Use search_nodes("keywords") to find related work
    ```
  </Tab>
