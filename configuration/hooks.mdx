...
Claude-mem integrates with Claude Code through a sophisticated hook system that automatically compresses transcripts, loads context, and manages memory across sessions. Hooks are configured in Claude Code's `settings.json` file and can be customized for different workflows.

## Hook Types

Claude-mem provides three main hooks that integrate with Claude Code's lifecycle:

### 1. SessionStart Hook
- **Triggered**: When a new Claude Code session begins
- **Purpose**: Loads relevant context from previous sessions
- **Location**: `~/.claude-mem/hooks/session-start.js`

### 2. PreCompact Hook
- **Triggered**: Before Claude Code compacts/clears a session
- **Purpose**: Compresses transcript into memory before data is lost
- **Location**: `~/.claude-mem/hooks/pre-compact.js`

### 3. SessionEnd Hook
- **Triggered**: When Claude Code session ends or `/clear` is used
- **Purpose**: Optionally saves memories before clearing context
- **Location**: `~/.claude-mem/hooks/session-end.js`

## Hook Configuration File

Hooks are configured in Claude Code's settings file at `~/.claude/.claude/settings.json`:
...
Create a `config.json` in the hooks directory to override CLI commands:
...
Hooks can check conditions before executing:
...
Hooks communicate with Claude Code using standardized JSON responses:
...
Hooks receive different payloads based on their type:
...
Claude-mem automatically installs hooks during setup:
