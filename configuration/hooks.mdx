...
You can integrate claude-mem with Claude Code through a sophisticated hook system that automatically compresses transcripts, loads context, and manages memory across your sessions. You configure hooks in Claude Code's `settings.json` file and can customize them for different workflows.

## Hook Types

You have access to three main hooks that integrate with Claude Code's lifecycle:
...
You configure hooks in Claude Code's settings file at `~/.claude/.claude/settings.json`:
